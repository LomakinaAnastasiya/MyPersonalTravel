...
class model_Admin extends Model
{
	...
	{	
		if ($_SESSION["Login"]=="Admin") {
	$db=Db::getConnection();	
	$str1='UPDATE `Zakaz` SET `Status`=1 where id='.$id;
	$result=$db->query($str1);
		}
	}
	...
	{	
		if ($_SESSION["Login"]=="Admin") {
	$db=Db::getConnection();	
	$str1='UPDATE `Zakaz` SET `Status`=2 where id='.$id;
	$result=$db->query($str1);
		}
	}


	function action_Red($num)
	{	
		if ($_SESSION['Login']=="Admin") {
			$db=Db::getConnection();
			$str1='UPDATE `Login` SET `Types`= 1 WHERE id='.$num;
			$result=$db->query($str1);
		}
	}

	function action_Uz($num)
	{	
		if ($_SESSION['Login']=="Admin") {
			$db=Db::getConnection();
			$str1='UPDATE `Login` SET `Types`= 2 WHERE id='.$num;
			$result=$db->query($str1);
		}
	}
